...
public class Context_Service extends Service implements SensorEventListener, MicrophoneRecorder.MicrophoneListener {
	...
	//Used for determining what activity the user is doing
	...
	class IncomingHandler extends Handler { 
		...
		public void handleMessage(Message msg) {
			...
			case MSG_START_ACCELEROMETER:
			{
				...
				long WINDOW_IN_MILLISECONDS = 5000;
	...
	/**
	 * This should classify and send to UI
	 * @param time
	 * @param accel
	 */
	private void classifyActivity(long time, float[] accel) {
	...
	private void sendSpeechStatusToUI(int speech) {
		for (int i=mClients.size()-1; i>=0; i-- ) {
			try {
				//Send Speech Status
				Message msg = Message.obtain(null, MSG_SPEECH_STATUS,speech,0);
				mClients.get(i).send(msg);
	...
	@Override
	/**
	 * This should classify microphone data and send to UI
	...
	public void microphoneBuffer(short[] buffer, int window_size) {
		...
		int voiced = 0;
		int isConversation = 0;
		int conversationThreshold = 55;

		for(int k=0;k<window_size-window_size/40;k+=window_size/80){ //Overlapping windows - move each window by 12.5-ms, don't go over buffer
			double[] features = FeatureExtractor.ComputeFeaturesForFrame(buffer,window_size/40,k); //Size of search is 25-ms (1-s/40)
				try {
				result=getObjectDoubleArray(features,result);
				double classId = SpeechDetector.classify(result);
				if (classId == 0.0) voiced++;
				if(voiced > conversationThreshold) break; //We already found voice, no need to keep searching
			} catch (Exception e) {
				e.printStackTrace();
			}
		}

		if(voiced > conversationThreshold) isConversation = 1;
		Log.i("microphone", voiced + "");
		sendSpeechStatusToUI(isConversation);
	}
