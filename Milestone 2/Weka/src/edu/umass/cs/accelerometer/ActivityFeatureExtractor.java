 ...
 * This class is responsible for extracting features over windows of the 
 * activity-labeled accelerometer data. Some of these features include 
 * mean and variance of individual axes, top FFT (Fast-Fourier Transform) 
 * frequencies, total energy, zero-crossing rate, etc. 
 * 
 * The most important parameter to this paradigm is the window_in_milliseconds
 * value which describes the size of the window in milliseconds. The default 
 * value is 5000 (5 seconds). This means you should have at a bare minimum 
 * 5 seconds of data for each activity. In reality you want at least several 
 * minutes of data. The more data, the better!
 * 
 * For the Activity Recognition assignment, you will use this java application 
 * for generating the .arff file (feature file) which will be used to train, 
 * evaluate and produce code for classifiers in WEKA. This file (or the 
 * previous version) will also be integrated in your My-Activities code so that 
 * features of windows can be collected for making predictions on the activity.
 */
public class ActivityFeatureExtractor {
	...
	private long window_in_milliseconds = 5000;
	 ...
	 */
	public ActivityFeatureExtractor(long window_in_milliseconds) {
		this.window_in_milliseconds = window_in_milliseconds;
	}
	...
	public Double[] extractFeatures(long timestamp, double ortAccX, double ortAccY, double ortAccZ,
		...
		if((timestamp-timeVector.get(0))< window_in_milliseconds)
			return null;
	...
	private void generateArffFile(String inputDir) {
		...
		String mergeFile = inputDir+"/ACCEL.csv";
		...
		try{
			...
			String label_line = "@attribute class{"+classes+"}\n";
			bw.write(label_line);
			bw.write("@data\n");

			br.close();
			br = new BufferedReader(new FileReader(mergeFile)); //reset
			while((s=br.readLine())!=null) {
				String tokens[] = s.split(",");
				if (tokens.length < 5)
					break;
	...
	public static void main(String args[]) {
		//TODO: Change to directory where ACCEL.csv is: (IT MUST BE CALLED ACCEL.csv)
		String INPUT_DIR = "/Users/stanleyrya/YourWorkingDir/390MB/Milestone 2/input_data";
		int window_size = 5000;
		ActivityFeatureExtractor afe = new ActivityFeatureExtractor(window_size);
		afe.generateArffFile(INPUT_DIR);
	}
